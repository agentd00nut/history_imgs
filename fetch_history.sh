...
function check_query_return_val(){
    ...
    if [ "${return_code}" -eq "0" ]; then
        ...
        echo "Something broke on the history query for ${browser}. nuts...";
    fi
}

if [ -e "${chrome_db}" ]; then
   
    sqlite3 "${chrome_db}" "SELECT url FROM urls ORDER BY last_visit_time DESC" > "data/chrome_default_history.data"
    return_code="$?";
...
if [ -e "${ff_db}" ]; then
    ...
    sqlite3 "${ff_db}" "SELECT url FROM moz_places JOIN moz_historyvisits ON place_id=moz_historyvisits.id" 2>/dev/null  > "data/ff_default_history.data"
    return_code="$?";
